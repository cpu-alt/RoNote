...
assert.equal(left.children[1].shadow.children[1].children[1].textContent,'5 000','value sits below the matching RAP row');
assert.equal(right.children[1].shadow.children[1].children[1].textContent,'4 000','opposite offer retains its own value');
shown.sides[0].items[0].rap = 1234;
a.pageItems[0][0] = { assetId: 20, value: null, noValue: true };
docListeners.scroll(); await flush();
assert.equal(left.children[1].shadow.children[1].children[1].textContent, '1 234', 'unvalued item displays current page RAP');
a.pageItems[0][0] = { assetId: 20, value: 5000 };
docListeners.scroll(); await flush();
assert.equal(left.children[1].shadow.children[1].children[1].textContent, '5 000', 'published value takes priority over RAP');
context.poll(); await flush();
...
assert.equal(totalRows[0].nextElementSibling.children[1].children[1].textContent, '5 000');
