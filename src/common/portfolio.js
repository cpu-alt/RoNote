...
/**
 * La correction mesuree par un rapport : ce qu'il faut ajouter aux chiffres
 * de Rolimon's pour compter les visages possedes en bundles et retirer les
 * fantomes. `null` si le rapport n'a pas pu reconcilier (source manquante,
 * inventaire prive).
 */
export function correctionOf(report) {
  if (!report?.ok || report.partial || !report.rolimons || !Number.isFinite(report.delta)) return null;
  const copies = (list) => (list || []).reduce((s, l) => s + (Number(l.count) || 0), 0);
  return {
    at: Number(report.at) || Date.now(),
    dv: report.value - report.rolimons.value,
    dr: report.rap - report.rolimons.rap,
    dn: copies(report.extras) - copies(report.ghosts)
  };
}

/**
 * La courbe du compte, bundles comptes : chaque releve de Rolimon's plus la
 * correction mesuree ce jour-la (la derniere connue a la fin de ce jour).
 *
 * RoNote ne mesure la correction que depuis qu'il tourne. Avant la premiere
 * mesure, on applique la plus ancienne connue et le point est marque `est` :
 * c'est une estimation — les bundles d'alors ne sont pas connus — que
 * l'interface trace en pointilles.
 *
 * @param points       [{at, v, r, n}]    serie de Rolimon's, triee
 * @param corrections  [{at, dv, dr, dn}] une par jour
 * @returns [{at, v, r, n, est}]
 */
export function correctedSeries(points, corrections) {
  const list = (corrections || []).filter(c => Number.isFinite(c?.at)).sort((a, b) => a.at - b.at);
  if (!list.length) return (points || []).map(p => ({ ...p, est: false }));
  const DAY = 864e5;
  let j = -1;
  return (points || []).map(p => {
    const endOfDay = (Math.floor(p.at / DAY) + 1) * DAY;
    while (j + 1 < list.length && list[j + 1].at < endOfDay) j++;
    const c = j >= 0 ? list[j] : list[0];
    return {
      ...p,
      v: Math.max(0, (p.v || 0) + (c.dv || 0)),
      r: Math.max(0, (p.r || 0) + (c.dr || 0)),
      n: Math.max(0, (p.n || 0) + (c.dn || 0)),
      est: j < 0
    };
  });
}
